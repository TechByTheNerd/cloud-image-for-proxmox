...
./prox-cloud-template-add-opensuse.sh ${HOST_DIGIT}01510 SSD-0${HOST_DIGIT}A Leap 15.1 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding 'openSUSE Leap 15.2'..."
./prox-cloud-template-add-opensuse.sh ${HOST_DIGIT}01520 SSD-0${HOST_DIGIT}A Leap 15.2 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding 'openSUSE Leap 15.3'..."
./prox-cloud-template-add-opensuse.sh ${HOST_DIGIT}01530 SSD-0${HOST_DIGIT}A Leap 15.3 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER
