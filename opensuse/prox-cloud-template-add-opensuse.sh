...
if [ ! -z "$3" ] ; then
    OPENSUSE_DISTRO="$3"
    OPENSUSE_DISTRO_LOWER=`echo $OPENSUSE_DISTRO | awk '{print tolower($0)}'`
else
    echo -e "ERROR: [distro] missing. (ex.: leap, tumbleweed, microos, kubic, etc)\n"
    exit -1
fi
if [ ! -z "$4" ] ; then
    OPENSUSE_VERSION="$4"
else
    echo -e "ERROR: [version] missing. (ex.: 15.4, etc). See: https://download.opensuse.org/repositories/Cloud:/Images:/Leap_15.4/images/\n"
    exit -1
fi
if [ ! -z "$5" ] ; then
    STD_USER_NAME="$5"
...
HASH_FILE="${OPENSUSE_DISTRO}_SHA256SUMS"

echo ""
echo "VM_ID................: $VM_ID"
echo "OPENSUSE_DISTRO......: $OPENSUSE_DISTRO"
echo "OPENSUSE_VERSION.....: $OPENSUSE_VERSION"
...
setStatus "STEP 1: Get openSUSE Cloud image and SHA256 hash" "*"
...
do
    ...
    setStatus "Downloading SHA256 sums from openSUSE (${HASH_URL})..." "*"
    if wget -q -N ${HASH_URL} --output-document=${HASH_FILE} ; then
        setStatus " - Extracting SHA256 hash from openSUSE (${HASH_FILE})..." "*"
...
if qm set ${VM_ID} --ciuser ${STD_USER_NAME} --cipassword ${STD_USER_PASSWORD} \
    ...
    --description "Virtual machine based on the openSUSE '${OPENSUSE_DISTRO}' Cloud \
...
echo "S U M M A R Y"
echo "======================================================================"
echo "New VM template based on openSUSE '$OPENSUSE_DISTRO' was created as VM_ID"
...
echo "======================================================================"
echo "openSUSE cloud images are using the following amount of space:"
echo ""
du -chs ./*.img ./*.qcow2
echo ""
