...
> Use a pre-configured Ubuntu Cloud image in your ProxMox Hypervisor.

This script is especially configured for exactly that. What the end-result is, is you get a VM "template" in ProxBox. You right-click on it and choose Clone, and can bring up a new VM that already has preconfigured:

- Hostname and DNS domain search name.
- Static IP address or DHCP.
- Non-root account that has `sudo` privilege.
- Your SSH keys added to `~/.ssh/authorized_keys`

So, it's just a few notches more-configured than if you installed the Ubuntu OS from scratch. These Ubuntu Cloud images are significantly smaller and stand up as quickly as you boot a VM. Meaning, from the moment you clone your new VM, it's up and available, and you can SSH in with your SSH keys, in the time it takes to boot! It's a pretty cool concept.
