...
./prox-cloud-template-add-centos.sh ${HOST_DIGIT}08000 SSD-0${HOST_DIGIT}A 8-stream $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding '9-stream'..."
./prox-cloud-template-add-centos.sh ${HOST_DIGIT}09000 SSD-0${HOST_DIGIT}A 9-stream $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER
