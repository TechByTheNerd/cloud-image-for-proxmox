...
./prox-cloud-template-add-ubuntu.sh ${HOST_DIGIT}02204 SSD-0${HOST_DIGIT}A jammy 22.04 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding 'Ubuntu 20.04 (focal)'..."
./prox-cloud-template-add-ubuntu.sh ${HOST_DIGIT}02004 SSD-0${HOST_DIGIT}A focal 20.04 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding 'Ubuntu 18.04 (bionic)'..."
./prox-cloud-template-add-ubuntu.sh ${HOST_DIGIT}01804 SSD-0${HOST_DIGIT}A bionic 18.04 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER

echo "[*] Rebuilding 'Ubuntu 16.04 (xenial)'..."
./prox-cloud-template-add-ubuntu.sh ${HOST_DIGIT}01604 SSD-0${HOST_DIGIT}A xenial 16.04 $ACCOUNT_NAME $PASSWORD $DOMAIN $LAUNCHPAD_USER
