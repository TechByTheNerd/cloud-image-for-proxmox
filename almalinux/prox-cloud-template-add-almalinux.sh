...
Name='ProxMox / AlmaLinux Cloud Init Image Creation Utility (PACIICU)'
...
then
    echo -e "USAGE: sudo $0 [id] [storage] [version] [user] [password] \
        [searchdomain] [sshkeyid]\n\n\tsudo $0 4000 SSD-01A 9 sysadmin \
...
DISK_SIZE="20G"
SSH_KEYS="./keys"
IMAGE_URL="https://repo.almalinux.org/almalinux/${ALMALINUX_VERSION}/cloud/x86_64/images/${IMAGE_FILE}"
HASH_URL="https://repo.almalinux.org/almalinux/${ALMALINUX_VERSION}/cloud/x86_64/images/"



echo ""
echo "VM_ID................: $VM_ID"
echo "ALMALINUX_VERSION....: $ALMALINUX_VERSION"
...
setStatus "STEP 1: Get AlmaLinux Cloud image and SHA256 hash" "*"
...
do
    ...
    setStatus "Downloading SHA256 sums from AlmaLinux (${HASH_URL}${HASH_FILE})..." "*"
    if wget -q -N ${HASH_URL}${HASH_FILE} --output-document=${IMAGE_FILE}.SHA256SUM ; then
        setStatus " - Extracting SHA256 hash from AlmaLinux (${IMAGE_FILE}.SHA256SUM)..." "*"
        SHA256_HASH_FROMINET=`grep "${IMAGE_FILE}" ./${IMAGE_FILE}.SHA256SUM | cut -d ' ' -f1 | xargs`
...
if qm create ${VM_ID} --memory ${MEM_SIZE} --name almalinux-cloud-${ALMALINUX_VERSION} \
    --net0 virtio,bridge=vmbr0 --tags almalinux,almalinux-${ALMALINUX_VERSION},cloud-image \
    --cpu cputype=host --watchdog model=i6300esb,action=reset; then
    setStatus " - Success." "s"
    setStatus " - NOTE: AlmaLinux 9 and later need the ProxMox CPU type to be 'host', else kernel panic."
else
...
if qm set ${VM_ID} --ciuser ${STD_USER_NAME} --cipassword ${STD_USER_PASSWORD} \
    ...
    --description "Virtual machine based on the AlmaLinux '${ALMALINUX_VERSION}' Cloud \
...
echo "S U M M A R Y"
echo "======================================================================"
echo "New VM template based on AlmaLinux '${ALMALINUX_VERSION}' was created as VM_ID"
