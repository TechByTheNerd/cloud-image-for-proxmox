...
Name='ProxMox / Debian Cloud Init Image Creation Utility (PUCIICU)'
...
then
    echo -e "USAGE: sudo $0 [id] [storage] [distro] [version] [user] [password] \
        [searchdomain] [sshkeyid]\n\n\tsudo $0 4000 SSD-01A Bullseye 11 sysadmin \
...
if [ ! -z "$3" ] ; then
    DEBIAN_DISTRO="$3"
    DEBIAN_DISTRO_LOWER=`echo $DEBIAN_DISTRO | awk '{print tolower($0)}'`
else
    echo -e "ERROR: [distro] missing. (ex.: Bullseye, Buster, etc)\n"
    exit -1
fi
if [ ! -z "$4" ] ; then
    DEBIAN_VERSION="$4"
else
    echo -e "ERROR: [version] missing. (ex.: 11, 10, etc)\n"
    exit -1
fi
if [ ! -z "$5" ] ; then
    STD_USER_NAME="$5"
...
IMAGE_FILE="debian-${DEBIAN_VERSION}-genericcloud-amd64.qcow2"
...
HASH_FILE="${IMAGE_FILE}_SHA512SUMS"
...
setStatus "STEP 1: Get Debian Cloud image and SHA512 hash" "*"
...
do
    ...
    if [ ! -f ./${IMAGE_FILE}-orig ]; then
        ...
        if [ "$HASHES_MATCH" -eq "0" ]; then
            setStatus " - SHA512 hashes do not match. File is invalid." "f"
    ...
    setStatus "Generating SHA512 hash from the file on-disk..." "*"
    SHA512_HASH_ONDISK=`sha512sum ./${IMAGE_FILE}-orig | cut -d ' ' -f1`
    setStatus " - Done: $SHA512_HASH_ONDISK" "s"

    setStatus "Downloading SHA512 sums from Debian (${HASH_URL})..." "*"
    if wget -q -N ${HASH_URL} --output-document=${HASH_FILE} ; then
        setStatus " - Extracting SHA512 hash from Debian (${HASH_FILE})..." "*"
        SHA512_HASH_FROMINET=`grep "${IMAGE_FILE}" ./${HASH_FILE} | cut -d ' ' -f1`
        setStatus " - Done: $SHA512_HASH_FROMINET" "s"
    else
        setStatus " - Download of SHA245 hashes failed." "f"
        exit -2
    fi

    setStatus "Comparing SHA512 hashes..." "*"
    if [[ "$SHA512_HASH_ONDISK" != "$SHA512_HASH_FROMINET" ]]; then
        HASHES_MATCH=0
    ...
    if [ $ATTEMPT -gt 3 ]; then
        setStatus "FATAL: Can't seem to download a valid image and confirm the \
            SHA512 hash after 3 attempts. Cannot continue." "f"
        exit -1
    fi
...
if qm set ${VM_ID} --ciuser ${STD_USER_NAME} --cipassword ${STD_USER_PASSWORD} \
    ...
    --description "Virtual machine based on the Debian '${DEBIAN_DISTRO}' Cloud \
...
echo "S U M M A R Y"
echo "======================================================================"
echo "New VM template based on Debian '$DEBIAN_DISTRO' was created as VM_ID"
...
echo "======================================================================"
echo "Debian cloud images are using the following amount of space:"
echo ""
du -chs ./*.qcow2
echo ""
